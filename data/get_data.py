...
def get_stock(path, number, time_=None):
    ...
    if time_:
        data = pro.daily(ts_code=number, start_date=time_['start_date'], end_date=time_['end_date'])
...
def get_all_data(number_list, path, start_time=None, end_time=None):
    for i in number_list:
        path_t = path + "/%s" % i
        print(path + "/%s" % i)
        check_dir_exist(path_t)
        if start_time and end_time:
            time_ = {'start_date': start_time,
                     'end_time': end_time}
            get_stock(path_t, i, time_=time_)
...
if __name__ == '__main__':
    ...
    stock_list = ['300730.SZ', '002628.SZ', '300144.SZ', '000017.SZ']
