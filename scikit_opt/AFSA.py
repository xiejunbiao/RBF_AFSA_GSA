...
class AFSA:
    def __init__(self, func, n_dim, size_pop=50, max_iter=300,
                 max_try_num=100, step=0.5, visual=0.3, bound1=0, bound2=1,
                 q=0.98, delta=0.5):
        self.func = func
        self.n_dim = n_dim
        ...
        self.bound = np.tile([[bound1], [bound2]], self.n_dim)
        self.X = self.generate()
        # self.X1 = np.random.rand(self.size_pop, self.n_dim)
        self.Y = np.array([self.func(x) for x in self.X])
        # print(self.Y)
        best_idx = self.Y.argmin()
    ...
    def generate(self):
        X = []
        for j in range(0, self.size_pop):
            rnd = np.random.random(size=self.n_dim)
            x_t = np.zeros(self.n_dim)
            for i in range(0, self.n_dim):
                # 生成一个位置
                x_t[i] = self.bound[0, i] + (self.bound[1, i] - self.bound[0, i]) * rnd[i]
            X.append(x_t)
        return np.array(X)
    def move_to_target(self, idx_individual, x_target):
        ...
        if self.Y[idx_individual] < self.best_Y:
            ...
            print('参数：%s' % self.best_x)
            print('最优适应度为：%s' % self.best_y)
